...
final class CementJobStage {
    ...
    // The inverse is defined in MudProperties.cementStages
    @Relationship var mud: MudProperties?

    // Back-reference to owning cement job
    @Relationship var cementJob: CementJob?
    ...
    init(name: String = "",
        ...
        if let cementJob {
            let next = ((cementJob.stages ?? []).map { $0.orderIndex }.max() ?? -1) + 1
